...
uint256 hashGenesisBlock("00000e8386f338d6b0db1c498d01622b0028370f256b56a93ec93ae8525443fa");
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!IsCoinBase())
        return 0;
    int d = GetDepthInMainChain();
    return max(0, (COINBASE_MATURITY+20) - d);
}
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    printf("AddToBlockIndex 22 \n");
    CTxDB txdb;
    txdb.TxnBegin();
    txdb.WriteBlockIndex(CDiskBlockIndex(pindexNew));

    printf("AddToBlockIndex 3 \n");
    // New best
    if (pindexNew->nHeight > nBestHeight)
    {
        printf("AddToBlockIndex 4 \n");
        if (pindexGenesisBlock == NULL && hash == hashGenesisBlock)
        {
            printf("AddToBlockIndex 41 \n");
            pindexGenesisBlock = pindexNew;
            txdb.WriteHashBestChain(hash);
        }
        else if (hashPrevBlock == hashBestChain)
        {
            printf("AddToBlockIndex 42 \n");
            // Adding to current best branch
            if (!ConnectBlock(txdb, pindexNew) || !txdb.WriteHashBestChain(hash))
            {
            printf("AddToBlockIndex 43 \n");
                txdb.TxnAbort();
        ...
        {
            printf("AddToBlockIndex 44 \n");
            // New best branch
            if (!Reorganize(txdb, pindexNew))
            {
            printf("AddToBlockIndex 45 \n");
                txdb.TxnAbort();
                return error("AddToBlockIndex() : Reorganize failed");
            }
        }

        printf("AddToBlockIndex 5 \n");
        // New best block
        hashBestChain = hash;
...
unsigned int calFirstBlockHash(CBlock* pblock)
{
    //
    ...
    loop
    {
        ...
        if(tmp.block.nNonce % 100000 == 0){
            printf("proof-of-work iter %d hash: %s  target: %s\n", tmp.block.nNonce, hash.GetHex().c_str(), hashTarget.GetHex().c_str());
        }
        if (hash <= hashTarget)
        {
            printf("calFirstBlockHash finish %d hash: %s  target: %s\n", tmp.block.nNonce, hash.GetHex().c_str(), hashTarget.GetHex().c_str());
            return tmp.block.nNonce;
        }
        tmp.block.nNonce++;
    }
}
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1231006505;
        // 超过0x00ffffff的数，会在getuint256的时候被截取
        // block.nBits    = 0x1d00ffff;
        block.nBits    = 0x1f0002ff;
        // block.nNonce   = 2083236893;
        block.nNonce   = 2085239325;
        block.nNonce = calFirstBlockHash(&block);
            //// debug print
...
void BitcoinMiner(bool newKey)
{
    ...
    printf("MakeNewKey finished newKey %d \n", newKey);
            ...
            if (hash <= hashTarget)
            {
                ...
                CRITICAL_BLOCK(cs_main)
                {
                    if (pindexPrev == pindexBest)
                    {
                        ...
                        printf("BitcoinMiner: newKey %d key %s \n", newKey, PubKeyToAddress(key.GetPubKey()).c_str());
                        if(newKey){
                            // Save key
                            if (!AddKey(key))
                                return;
        ...
        CRITICAL_BLOCK(cs_mapWallet)
        {
            ...
            loop
            {
                ...
                if (!SelectCoins(nTotalValue, setCoins)){
                    printf("SelectCoins nTotalValue %d failed \n", nTotalValue);
                    return false;
                }
                int64 nValueIn = 0;
                foreach(CWalletTx* pcoin, setCoins){
                    int credit = pcoin->GetCredit();
                    nValueIn +=  credit;
                    printf("CreateTransaction walletTX hash %s credit %d \n", pcoin->ToString().c_str(), credit);
                }
                printf("CreateTransaction vValueIn %d nTotalValue %d \n", nTotalValue, nTotalValue);
