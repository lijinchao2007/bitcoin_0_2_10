...
bool CMyApp::OnInit2()
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("--help"))
    {
        wxString strUsage = string() +
          ...
          _("Options:\n") +
            ...
            "  -newkey=1          \t  " + _("Don't generate coins\n") +
            "  -min            \t  " + _("Start minimized\n") +
            "  -datadir=<dir>  \t  " + _("Specify data directory\n") +
            "  -proxy=<ip:port>\t  " + _("Connect through socks4 proxy\n") +
            "  -port=8333          \t  " + _("Don't generate coins\n") +
    ...
    if (mapArgs.count("-port")){
        port = atoi(mapArgs["-port"].c_str());
    }
    if (!BindListenPort(port, strErrors))
    {
    ...
    if(mapArgs.count("-newkey")){
        if (mapArgs["-newkey"].empty())
            fNewKey = true;
        else
            fNewKey = (atoi(mapArgs["-gen"].c_str()) != 0);
    }
